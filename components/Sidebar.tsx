...
const Sidebar: React.FC<SidebarProps> = ({ mobileOpen, setMobileOpen }) => {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed top-0 left-0 z-50 h-screen bg-white dark:bg-dark-900 border-r border-slate-200 dark:border-white/[0.06] transition-all duration-500 ease-[cubic-bezier(0.25,0.1,0.25,1)] flex flex-col
          ${mobileOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}
          ${collapsed ? 'w-20' : 'w-72'}
        `}
      >
        {/* Header */}
        <div className={`h-20 flex items-center ${collapsed ? 'justify-center' : 'justify-between px-6'} border-b border-slate-100 dark:border-white/[0.06] shrink-0`}>
          <Link
            to="/"
            onClick={() => setMobileOpen(false)}
            className="group"
          ...
          {!collapsed && (
            <button
              ...
              className="hidden md:flex p-1.5 rounded-full text-slate-400 hover:bg-slate-100 dark:hover:bg-white/[0.06] hover:text-slate-900 dark:hover:text-white transition-colors"
            >
              <ChevronLeft size={16} />
            </button>
          )}
        ...
        {collapsed && (
          <button
            ...
            className="hidden md:flex absolute top-7 -right-3 z-50 p-1 bg-white dark:bg-dark-800 border border-slate-200 dark:border-white/[0.08] text-gold-500 rounded-full shadow-md hover:scale-110 transition-transform"
          >
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide py-6 px-3 space-y-8">
            ...
            {!collapsed ? (
              <div className="relative group">
                <input
                  type="text"
                  placeholder="Search..."
                  className="w-full bg-slate-50 dark:bg-dark-800 border border-slate-200 dark:border-white/[0.08] rounded-md py-2.5 pl-9 pr-4 text-xs font-medium text-slate-900 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-gold-500/50 focus:border-gold-500/50 transition-all"
                />
                <Search className="absolute left-3 top-2.5 text-slate-400 group-focus-within:text-gold-500 transition-colors" size={14} />
              </div>
            ) : (
              <button className="p-2.5 rounded-md hover:bg-slate-100 dark:hover:bg-white/[0.06] text-slate-400 hover:text-gold-500 transition-colors" title="Search">
          ...
          <div>
            {!collapsed && (
              <h3 className="px-3 text-[10px] font-bold uppercase tracking-[0.2em] text-slate-500 dark:text-slate-500 mb-3 flex items-center gap-2">
                <Briefcase size={10} className="text-gold-600/80" /> Wealth
              </h3>
            )}
          ...
          <div>
            {!collapsed && (
              <h3 className="px-3 text-[10px] font-bold uppercase tracking-[0.2em] text-slate-500 dark:text-slate-500 mb-3 flex items-center gap-2">
                <Compass size={10} className="text-gold-600/80" /> Wander
              </h3>
            )}
            <div className="space-y-1">
        ...
        {/* Footer / Theme Toggle */}
        <div className="p-4 border-t border-slate-100 dark:border-white/[0.06] bg-slate-50/50 dark:bg-dark-950/50">
